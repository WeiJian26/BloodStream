...
if($r== 1){
    ...
    echo 'document.location.href = "signup_hospital.php";';
    echo '</script>';

}else{

    $addnewuser= " INSERT INTO user (email, password, fullname, address, phoneNumber, userType) 
    value ('$email' , '$password' , '$fullname' , '$address' , '$phoneNumber', 'Hospital')";
    mysqli_query($con, $addnewuser);
    $userID=$con->insert_id;

    $addNewHospital =  "INSERT INTO hospital (userID,state, regNum,opStat) value ('$userID','$state' ,'$regNum' ,'$opStat')";
